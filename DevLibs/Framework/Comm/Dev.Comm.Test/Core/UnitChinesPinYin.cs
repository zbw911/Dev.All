...
namespace Dev.Comm.Test.Core
{
    ...
    public class UnitChinesPinYin
    {
        ...
        {
            var pn = ChineseToBopomofo.ConvertToBopomofo(yx);
        ...
        {
            var pn = ChineseToBopomofo.ConvertToBopomofo(cs);

            Assert.AreEqual("chuanshuo", pn);
        }

        [TestMethod]
        public void TestAllChineseChar()
        {
            string[] strChineseCharList =
            {
                #region 
            ...
            var unSupportcount = 0;
            foreach (var s in strChineseCharList)
            {
                var s2 = s.Substring(1);
                var firstcode = s.Substring(0, 1).ToLower();
                foreach (var sin in s2)
                {
                    var chschar = sin.ToString();

                    var firstpinyin = ChineseToBopomofo.GetFirstPinYin(chschar);

                    if (firstpinyin != firstcode)
                    {
                        unSupportcount++;
                    }
                }
            ...
            Console.WriteLine("All=>" + strChineseCharList.Sum(x => (x.Count() - 1)));
            Console.WriteLine("UnSupport=>" + unSupportcount);
