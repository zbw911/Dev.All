...
//  文件名：Dev.Libs/Dev.Comm.Core/IOUtility.cs
//  
...
namespace Dev.Comm.IO
{
    public static class IOUtility
    {
        ...
        public static void DeleteDirectory(string physicalPath, bool recursive)
        {
            ...
            try
            {
                if (Directory.Exists(physicalPath))
                {
                    try
                    {
                        FileSystem.DeleteDirectory(physicalPath, UIOption.OnlyErrorDialogs,
        ...
        public static IEnumerable<DirectoryInfo> EnumerateDirectoriesExludeHidden(string path)
        {
            ...
            return
                dir.EnumerateDirectories().Where(it => (it.Attributes & FileAttributes.Hidden) != FileAttributes.Hidden);
        }
        ...
        public static IEnumerable<FileInfo> EnumerateFilesExludeHidden(string path, string searchPattern)
        {
            var dir = new DirectoryInfo(path);
            return
                dir.EnumerateFiles(searchPattern).Where(
                    it => (it.Attributes & FileAttributes.Hidden) != FileAttributes.Hidden);
        }
    }
}