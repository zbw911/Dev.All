...
namespace Application.MainBoundedContext
{
    public class SystemMessagerManager
    {
        ...
        private static string SmsApi = CommConfiguration.Config.SmsApi;
        /// <summary>
        /// 发送激活邮箱用的邮件
        /// </summary>
        ...
        public static bool SendValidateMail(string email, string nickname, string Subject, string ContentMessage)
        {
            ...
            {
                //邮箱格式不正确
                Dev.Log.Loger.Error("【Dx.Activity.BLL.Members--Add()--SendValidateMail()】账号已经注册成功，发送激活邮件失败<验证邮箱格式报错了Dev.Comm.Validate.Validate.IsEmail(email)>");

                return false;
            }

            //邮箱设置
            string configMessage = ConfigurationManager.AppSettings["ToMail"].ToString();

            Dev.Comm.Net.Mail mymail = new Dev.Comm.Net.Mail("");
            string result;
            int around = mymail.ToSendMail(configMessage, email, nickname, Subject, ContentMessage, out result);
        ...
        public static string GetContentForGetPass(string name, string token)
        {
            ...
            //激活链接
            string Tourl = indexURL + "/Account/RestPwd?token=" + token;

            //邮件内容
            ContentMessage += "<table width=100% border=0 bgcolor=#F5F5F5 cellpadding=5 cellspacing=1 style=font-size:12px>";
            ...
            ContentMessage += "<tr><td>(此链接48小时内有效，如果无法点击该URL链接地址，请将它复制并粘帖到浏览器的地址输入框，然后单击回车即可。如有疑问,请联系游戏团客服：0311-88611797)</td></tr>";
        ...
        public static string ActMessage(string email, string nickname, string token)
        {
            ...
            //激活链接
            string Tourl = indexURL + "Account/Activation?token=" + token;
            ContentMessage += "<table width=100% border=0 bgcolor=#F5F5F5 cellpadding=5 cellspacing=1 style=font-size:12px>";
            ...
            ContentMessage += "<tr><td><a href=" + Tourl + ">" + Tourl + "</a></td></tr>";
            ContentMessage += "<tr><td>(此链接有效时间为48小时，如果无法点击该URL链接地址，请将它复制并粘帖到浏览器的地址输入框，然后单击回车即可。如有疑问,请联系游戏团客服：0311-88611797)</td></tr>";
            ContentMessage += "<tr><td>&nbsp;&nbsp;感谢您对游戏团的支持，若有疑问请联系客服\a此为系统自动发出，请不要回复本邮件。</td></tr></table>";
            return ContentMessage;
        }
        ...
        public static string OpenMail(string mail)
        {
            ...
            var result = "";
            string[] str_list = mail.Trim().Split('@');
            if (str_list.Length > 1)
            {
                string[] mail_ads = str_list[1].Split('.');
        ...
        public static bool SendSMS(string pnum, string message, decimal uid)
        {
            ...
            string sendMessage = Dev.Comm.Net.Http.GetUrl(SmsApi
                + "/api/kt_api.aspx?method=SendMessage&pNum=" + pnum + "&message=" + "code" + "&sendmsg=" + (msg) + "&SType=" + "5" + "&Uid=" + uid);
