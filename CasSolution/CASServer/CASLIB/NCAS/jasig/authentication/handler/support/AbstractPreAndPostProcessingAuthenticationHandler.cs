...
namespace NCAS.jasig.authentication.handler.support
{
        ...
        : NamedAuthenticationHandler
    {
        ...
        //@NotNull
        private string name = typeof(AbstractPreAndPostProcessingAuthenticationHandler).FullName;// getClass().getName();
     ...
     */
        ...
        public bool authenticate(Credentials credentials)
        {
            ...
            if (!this.preAuthenticate(credentials))
            {
                return false;
            }

            bool authenticated = this.doAuthentication(credentials);

            return this.postAuthenticate(credentials, authenticated);
        }
